...
update_plugins := cd .. && notepad.vim/update.sh
...
install: install.sh installer_bin.sh installer_first_help
	mkdir -p "$(INSTALL_PATH)"/bundle
	cd "$(INSTALL_PATH)"/bundle && ln -s "$(CURDIR)" .
	sh installer_bin.sh "$(INSTALL_PATH)" "$(INSTALL_BIN_PATH)" bin/*
	$(ask_for_updating_plugins) && cd "$(INSTALL_PATH)"/bundle/notepad.vim && $(update_plugins)
	echo
